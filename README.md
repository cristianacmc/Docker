...
```
docker run -d --network app-network -p 80:80 --mount type=bind,source=$(pwd)/nginx.conf,target=/etc/nginx/nginx.conf nginx
```

### Trio Task Tutorial
This exercise gets you to run a Flask application container, a MySQL container and an NGINX container in a network.

1 - Containerise the MySQL database and the Flask application from the GitLab repo
```
...
```
3 - Look up the MySQL 5.7 image documentation on Docker Hub to learn how to configure the database
```

```
4 - Create a network and connect the Flask application container, database container
```

```
5 - Create an NGINX container to and connect it to the network.
You will need to either bind mount the nginx.conf file to /etc/nginx/nginx.conf on the container, or create a custom NGINX image which copys the file there instead.
```
